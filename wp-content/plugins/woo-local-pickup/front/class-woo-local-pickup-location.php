...
class Woo_Local_Pickup_Location {
	...
	public function woo_pickup_location_field( $checkout ) {
		...
		if(!empty($locations))
		{
			echo '<p class="form-row" style="'.$style.'" id="pickup_location_field"><label for="pickup_location_field" class="">'.__( '取貨點', $this->plugin_slug ).'<abbr class="required" title="required">*</abbr></label><select name="user_pickup_location" id="user_pickup_location">
 				<option value="">'.__( '選擇…', $this->plugin_slug ).'</option>';
	...
	public function woo_pickup_field_validate() {
		...
		if(!empty($matches))
		{
			...
			wc_add_notice(__( '您未指定取貨點/Pickup Location is a required field.', $this->plugin_slug ),'error');
		}
	...
	public function woo_update_order_meta( $order_id ) {
		if ( $_POST['user_pickup_location'] ) 
		{
			update_post_meta( $order_id, 'user_pickup_location', $_POST['user_pickup_location'] );
			$address = get_post_meta($_POST['user_pickup_location'],'pickup_location',true);
			update_post_meta( $order_id, 'user_pickup_location_address',  $address);
		}
	}
	...
	public function woo_update_order_email( $keys ) {
		$keys['Pickup Location'] = 'user_pickup_location_address';
		return $keys;
	}
